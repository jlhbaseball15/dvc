...
def run(
    ...
    **kwargs,
    ...
    hydra_sweep = any(
        x.is_sweep_override()
        for x in to_hydra_overrides(path_overrides.get(hydra_output_file, []))
    )
    ...
    if queue:
        if hydra_sweep:
            ...
            sweeps = get_hydra_sweeps(path_overrides, hydra_output_file)
        else:
        ...
        for sweep_overrides in sweeps:
            ...
            if sweep_overrides is not None:
                ui.write(f"Queueing with '{sweep_overrides}'.")
